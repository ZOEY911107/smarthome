Page({
  data: {
    ...
    button_viwe_display: "block", //前端按钮展示状态
...
//小程序调用拍照功能
  chooseWxImage: function (type) {
    var myThis = this;
    myThis.setData({
      ImagetempFilePaths: "" //将选择图片后的临时地址写给ImagetempFilePaths等待其他函数调用 
    })
    wx.chooseImage({
      sizeType: ['compressed'],
      sourceType: [type],
      success: function (chooseImage_res) {
        console.log(chooseImage_res);
        wx.getImageInfo({ //图片属性接口
          src: chooseImage_res.tempFilePaths[0], //地址为选择图片后在本地的临时文件
          success(getImageInfo_res) {
            var ctx_size = 200 / getImageInfo_res.width;
  ...
  chooseimage: function () {
    ...
    wx.showActionSheet({
      ...
      success: function (res) {
        if (!res.cancel) {
          if (res.tapIndex == 0) {
            myThis.chooseWxImage('album')
          } else if (res.tapIndex == 1) {
            myThis.chooseWxImage('camera')
          }
        }
        wx.hideLoading(); //关闭加载中弹窗
      }
 ...
 CreatePerson(){
  ...
  wx.showActionSheet({
             ...
             console.log("临时地址:" + chooseImage_res.tempFilePaths[0])
              ...
              const uploadTask = wx.cloud.uploadFile({ //云存储上传接口
                ...
                success(uploadFile_res) {
                 ...
                 console.log("ImageFileID:" + uploadFile_res.fileID)
                  wx.hideLoading() //关闭加载中弹窗
                  wx.showToast({ //显示弹窗
                    title: '上传成功',
                    icon: 'success',
   ...
   ImagetempFilePaths: chooseImage_res.tempFilePaths[0] //将选择图片后的临时地址写给ImagetempFilePaths等待其他函数调用
              ...
              console.log("ImagetempFilePaths:" + myThis.data.ImagetempFilePaths)
              const uploadTask = wx.cloud.uploadFile({ //云存储上传接口
                cloudPath: random + '.png', //将图片上传后名称为随机数 + .png
                filePath: chooseImage_res.tempFilePaths[0], //将临时地址中的图片文件上传到云函数文件服务器
                success(uploadFile_res) {
  ...
  CreateFace() { //人脸检测函数
    ...
    var myThis = this
    myThis.setData({
      text_viwe_display: "none" //展示人脸检测相关数据
    });
    ...
    wx.cloud.callFunction({ //人脸检测云函数
      name: 'GetPersonListNum',
      data: {
        "GroupId": "1"
      },
      success(cloud_callFunction_res) {
        console.log("result:" + JSON.stringify(cloud_callFunction_res.result)) //人脸检测返回的json数据
        var personnum = cloud_callFunction_res.result.PersonNum + 1;
        myThis.setData({
          PersonNum: personnum //展示人脸检测相关数据
        });
        console.log(myThis.data.PersonNum)
        wx.cloud.callFunction({ //人脸检测云函数
          name: 'CreatePerson',
          data: {
            fileID: myThis.data.ImageFileID,//上传成功文件的fileID
            PersonName: myThis.data.PersonName,
            PersonId: myThis.data.PersonNum,
            Gender: 0
...
FaceLock(){
  ...
  wx.chooseImage({ //选择图片接口
    ...
    success(chooseImage_res) {
      ...
      console.log("临时地址:" + chooseImage_res.tempFilePaths[0])
      wx.cloud.callFunction({ //获取人脸库人员数
        name: 'GetPersonListNum',
        data: {
          "GroupId": "1"
        },
        success(cloud_callFunction_res) {
          console.log("result:" + JSON.stringify(cloud_callFunction_res.result)) //人脸检测返回的json数据
          myThis.setData({
            PersonNum: cloud_callFunction_res.result.PersonNum //展示人脸检测相关数据
          });
          console.log(myThis.data.PersonNum)
          wx.getFileSystemManager().readFile({
            filePath: myThis.data.ImagetempFilePaths, //选择图片返回的相对路径
            encoding: 'base64', //设置编码格式为base64
            success(base64_res) {
              console.log(base64_res)
         myThis.Verify(base64_res.data)   
            }
          })
        },
        fail(err) { //失败回调函数
 ...
 Verify(Image){
  ...
  var myThis = this
  var i = myThis.data.PersonNum
  for (i; i > 0; i--) {
    if (!myThis.data.IsMatch) {
      myThis.setData({
        PersonId: i //展示人脸检测相关数据
      });
      console.log(myThis.data.PersonId)
      wx.cloud.callFunction({ //人脸检测云函数
        name: 'VerifyFace',
        data: {
          "Image": Image,
          "PersonId": myThis.data.PersonId
        },
        success(cloud_callFunction_res) {
          wx.hideLoading()
          console.log("result:" + JSON.stringify(cloud_callFunction_res.result)) //人脸检测返回的json数据
          console.log(cloud_callFunction_res.result.IsMatch)
          if (cloud_callFunction_res.result.IsMatch) {
          
            wx.showModal({
              title: '成功',
              content: "相似度：" + cloud_callFunction_res.result.Score
            });
            myThis.setData({
              PersonId: "",
              IsMatch: cloud_callFunction_res.result.IsMatch,
              Score: cloud_callFunction_res.result.Score
            });
            console.log(myThis.data.IsMatch)
          }
        },
        fail(err) { //失败回调函数
          console.log(err)
