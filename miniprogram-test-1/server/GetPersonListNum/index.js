...
var synGetPersonListNum = function (GroupId) { //人脸识别API
  ...
  let req = new models.GetPersonListNumRequest();
  
  let params = '{"GroupId":"' + GroupId + '"}' //拼接参数
  req.from_json_string(params);
  return new Promise(function (resolve, reject) { //构造异步函数
    client.GetPersonListNum(req, function (errMsg, response) {
      if (errMsg) {
        reject(errMsg)
      } else {
        resolve(response);
...
exports.main = async (event, context) => {
  ...
  datas = await synGetPersonListNum(event.GroupId) //调用异步函数，向腾讯云API发起人脸检测请求
  return datas
}